...
class FieldWorkerPendingFlatsNew extends StatefulWidget {
  final String? highlightPropertyId;

  const FieldWorkerPendingFlatsNew({super.key, this.highlightPropertyId});

  @override
  State<FieldWorkerPendingFlatsNew> createState() => _FieldWorkerPendingFlatsNewState();
}

class _FieldWorkerPendingFlatsNewState extends State<FieldWorkerPendingFlatsNew> {
    ...
    try {
      ...
      if (resp.statusCode == 200) {
        ...
        if (context.mounted) {
          ...
          if (context.findAncestorStateOfType<_FieldWorkerPendingFlatsNewState>() != null) {
            final parentState = context.findAncestorStateOfType<_FieldWorkerPendingFlatsNewState>()!;
                              ...
                              TextButton.icon(
                                ...
                                onPressed: () {
                                  Navigator.of(context).push(MaterialPageRoute(builder: (context) {
                                    return AdministatorPropertyDetailPage(propertyId: item.pId.toString());
                                  }));
                                },
                              ),
                            ],
...
Widget _buildAmountRowMini(String label, dynamic value, Color color,BuildContext context) {
...
Widget _buildPaymentStepCard(String title, FirstPaymentRecord record,BuildContext context,{
  String? status,
  String? dateTime,
  ...
  return Container(
    ...
    child: Column(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        ...amounts.map((amount) => _buildAmountRowMini(amount.label, amount.value, amount.color,context
        )),
      ],
    ),
  );
