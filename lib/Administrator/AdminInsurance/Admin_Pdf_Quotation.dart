...
class AdminInsuranceProject {
  ...
  AdminInsuranceProject.fromJson(Map<String, dynamic> j)
      : id = j['id'],
        name = j['name_'] ?? '',
        number = j['number'] ?? '',
...
Future<AdminInsuranceProject> fetchProject(int id) async {
  final uri = Uri.parse(
  ...
  return AdminInsuranceProject.fromJson(json['data'][0]);
}
...
// ─── MAIN SCREEN ─────────────────────────────────────────────────────────────
class Admin_PdfQuotation extends StatefulWidget {
  final int projectId;
  const Admin_PdfQuotation({super.key, required this.projectId});

  @override
  State<Admin_PdfQuotation> createState() => _Admin_PdfQuotationState();
}

class _Admin_PdfQuotationState extends State<Admin_PdfQuotation> {
  late Future<AdminInsuranceProject> _future;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      //     MaterialPageRoute(
      //       builder: (_) =>
      //           InsurancePlansScreen(Id: widget.projectId.toString()),
      //     ),
      //   ),
      ...
      body: FutureBuilder<AdminInsuranceProject>(
        future: _future,
        builder: (ctx, snap) {
...
class _ProjectBody extends StatelessWidget {
  final AdminInsuranceProject project;
