...
import 'Admin_Expire_aggrement_details.dart';
...
class AgreementApiService {
  ...
  static Future<ExpireAgreementResponse> fetchExpiredAgreements({
    ...
    final uri = Uri.parse(
      '$_baseUrl/agreement_expire.php?Fieldwarkarnumber=$number',
    );
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return ExpireAgreementResponse.fromJson(jsonMap);
      } else {
...
class AdminExpireAgreementScreen extends StatefulWidget {
  final String fieldWorkerNumber;

  const AdminExpireAgreementScreen({
    super.key,
    required this.fieldWorkerNumber,
  });

  @override
  State<AdminExpireAgreementScreen> createState() =>
      _AdminExpireAgreementScreenState();
}

class _AdminExpireAgreementScreenState
    extends State<AdminExpireAgreementScreen> {
  ...
  final Map<String, List<ExpireAgreementData>> _groupedData = {};
    ...
    try {
      ...
      if (_isAdmin || _isSubAdmin) {
        ...
        for (final fw in _allowedWorkers) {
          _groupedData[fw['id']!] = [];
          try {
            final response = await AgreementApiService.fetchExpiredAgreements(
                number: fw['id']!);
                    ...
                    child: ElevatedButton.icon(
                      onPressed: () {
                        Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder: (_) =>
                                ExpireAgreementDetailPage(data: data),
                          ),
                        );
                      },
