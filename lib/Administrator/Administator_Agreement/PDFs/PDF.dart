...
Future<pw.ImageProvider?> _loadAgreementImage(String path) async {
  ...
  try {
    ...
    if (res.statusCode == 200 && res.bodyBytes.isNotEmpty) {
      ...
      // ✅ Re-encode as JPEG with compression (quality 70)
      final compressedBytes = img.encodeJpg(grayscale, quality: 70);

      return pw.MemoryImage(Uint8List.fromList(compressedBytes));
    }
  } catch (_) {}
  return null;
}
...
Future<File> generateAgreementPdf(Map<String, dynamic> data) async {
  ...
  pw.TextSpan getMeterClause(Map<String, dynamic>? data) {
    ...
    final meterValue =
    (customMeterUnit != null && customMeterUnit.toString().isNotEmpty)
  ...
  (agreement_type.trim().toLowerCase() == "renewal agreement") ? 4 : 5;

  final electricityClauseNumber =
  (agreement_type.trim().toLowerCase() == "renewal agreement") ? 5 : 6;
  ...
  pdf.addPage(
    pw.MultiPage(
      ...
      build: (context) => [
          ...
          text: pw.TextSpan(children: [
            pw.TextSpan(
                ...
                style: pw.TextStyle(
                    fontWeight: pw.FontWeight.bold, fontSize: 11)),
            pw.TextSpan(
                text: 'A security deposit of ', style: baseStyle),
            pw.TextSpan(
        ...
        pw.SizedBox(height: kNormalSpace),
  ...
  pdf.addPage(
    pw.MultiPage(
      ...
      build: (context) => [
          ...
          text: pw.TextSpan(
            children: [
              ...
              if (additionalTenants.isEmpty) ...[
                ...
                pw.TextSpan(
                    text: tenantRelationPerson, style: boldStyle),
              ] else ...[
        ...
        if (additionalTenants.isEmpty)
          pw.Text(
              'My permanent address is: $tenantPermAddress',
              style: boldStyle)
        else ...[
          ...
          pw.SizedBox(height: 6),
          pw.Text(
              '1. $tenantName - $tenantPermAddress',
              style: boldStyle),
...
// ─── LOAD AADHAR IMAGES ───────────────────────────────────────────
  final ownerFrontImg  = await _loadAgreementImage(data['owner_aadhar_front']?.toString()  ?? '');
  final ownerBackImg   = await _loadAgreementImage(data['owner_aadhar_back']?.toString()   ?? '');
  final tenantFrontImg = await _loadAgreementImage(data['tenant_aadhar_front']?.toString() ?? '');
  final tenantBackImg  = await _loadAgreementImage(data['tenant_aadhar_back']?.toString()  ?? '');

  // ─── PAGE — AADHAR IMAGES (Owner + Tenant same page) ─────────────
  pdf.addPage(
    pw.Page(
      pageFormat: PdfPageFormat.a4,
      ...
      build: (_) => pw.Column(
        ...
        children: [
          ...
          pw.Row(
            children: [
              pw.Expanded(
                child: ownerFrontImg != null
                    ? pw.Image(ownerFrontImg, fit: pw.BoxFit.contain)
                    : pw.Container(height: 180, color: PdfColors.grey300,
                    child: pw.Center(child: pw.Text('Not Available'))),
              ),
              pw.SizedBox(width: 8),
              pw.Expanded(
                child: ownerBackImg != null
                    ? pw.Image(ownerBackImg, fit: pw.BoxFit.contain)
                    : pw.Container(height: 180, color: PdfColors.grey300,
                    child: pw.Center(child: pw.Text('Not Available'))),
              ),
            ],
          ...
          pw.Row(
            children: [
              pw.Expanded(
                child: tenantFrontImg != null
                    ? pw.Image(tenantFrontImg, fit: pw.BoxFit.contain)
                    : pw.Container(height: 180, color: PdfColors.grey300,
                    child: pw.Center(child: pw.Text('Not Available'))),
              ),
              pw.SizedBox(width: 8),
              pw.Expanded(
                child: tenantBackImg != null
                    ? pw.Image(tenantBackImg, fit: pw.BoxFit.contain)
                    : pw.Container(height: 180, color: PdfColors.grey300,
                    child: pw.Center(child: pw.Text('Not Available'))),
              ),
            ],
  ...
  // ─── ADDITIONAL TENANTS PAGE (only if they exist) ─────────────────
  if (additionalTenants.isNotEmpty) {
    for (final t in additionalTenants) {

      final String tFrontPath = t['tenant_aadhar_front']?.toString() ?? '';
      final String tBackPath  = t['tenant_aadhar_back']?.toString()  ?? '';
          ...
          build: (_) => pw.Column(
            ...
            children: [
              pw.Row(
                children: [
                  pw.Expanded(
                    child: tFrontImg != null
                        ? pw.Image(tFrontImg, fit: pw.BoxFit.contain)
                        : pw.Container(height: 180, color: PdfColors.grey300,
                        child: pw.Center(child: pw.Text('Not Available'))),
                  ),
                  pw.SizedBox(width: 8),
                  pw.Expanded(
                    child: tBackImg != null
                        ? pw.Image(tBackImg, fit: pw.BoxFit.contain)
                        : pw.Container(height: 180, color: PdfColors.grey300,
                        child: pw.Center(child: pw.Text('Not Available'))),
                  ),
                ],
  ...
  final file =
  File('${dir.path}/agreement_${data['id'] ?? 'unknown'}.pdf');
