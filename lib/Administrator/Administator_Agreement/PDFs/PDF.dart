...
DateTime addMonthsSafely(DateTime from, int monthsToAdd) {
  ...
  final newDay =
  from.day <= lastDayOfNewMonth ? from.day : lastDayOfNewMonth;
  return DateTime(newYear, newMonth, newDay);
}

/* -------------------- IMAGE LOADER -------------------- */
Future<pw.ImageProvider?> _loadAgreementImage(String path) async {
  ...
  try {
    ...
    final res = await http.get(Uri.parse(fullUrl)).timeout(
      const Duration(seconds: 15),
      onTimeout: () => http.Response('', 408),
    );

    print(
        '📡 Status: ${res.statusCode} | Size: ${res.bodyBytes.length} bytes | Path: $path');

    if (res.statusCode == 200 && res.bodyBytes.isNotEmpty) {
      return pw.MemoryImage(res.bodyBytes);
    } else {
      print(
          '❌ Image load failed: status=${res.statusCode} path=$path');
    }
  } catch (e) {
    print('❌ Image load exception: $e | path=$path');
  }
  return null;
}

/* -------------------- SAFE TEXT CLEANER -------------------- */
// Replaces special Unicode characters that pdf package can't render
String safeCleanText(String input) {
  return input
      .replaceAll('\u2014', '-')   // em dash —
      .replaceAll('\u2013', '-')   // en dash –
...
/* ─── Safe Text Cleaner ──────────────────────────────────────────
   Replaces Unicode chars that the pdf package default font can't render.
   Call this on ANY user-typed string before passing to pw.Text / pw.TextSpan.
─────────────────────────────────────────────────────────────────── */

/* -------------------- GLOBAL CLAUSE COUNTER -------------------- */
late int _clauseCounter;

/* -------------------- CLAUSE WIDGET -------------------- */
pw.Widget clause(String title, String body) {
  final number = _clauseCounter++;
  ...
  return pw.Partition(
    child: pw.Padding(
      ...
      child: pw.RichText(
        text: pw.TextSpan(
          style: pw.TextStyle(fontSize: 10.5, height: 1.4),
          children: [
            pw.TextSpan(
              text: '$number. ',
              style: pw.TextStyle(
...
// ─── Custom Clause Widget ─────────────────────────────────────────
// Same _clauseCounter → numbering continues from last default clause.
// safeCleanText() strips special Unicode so PDF renders correctly.
pw.Widget customClausePdf(String title, String subtitle) {
  final number = _clauseCounter++;
  final cleanTitle    = safeCleanText(title.trim());
  final cleanSubtitle = safeCleanText(subtitle.trim());

  // Debug — verify data reaches PDF layer
  print('📄 [PDF] Custom Clause $number | title: "$cleanTitle" | body(${cleanSubtitle.length} chars)');
        ...
        text: pw.TextSpan(
          ...
          children: [
            ...
            if (cleanSubtitle.isNotEmpty)
              pw.TextSpan(text: cleanSubtitle),
          ],
        ),
      ),
    ),
...
pw.Widget _buildPageSignatures(
  ...
  context.pageNumber == 0 ||
      context.pageNumber == context.pagesCount - 1
  ) {
    return pw.Container(
...
Future<File> generateAgreementPdf(
    Map<String, dynamic> data, {
      List<Map<String, String>>? customClauses,
    }) async {
  ...
  final ownerRelationPerson =
  safeString(data, 'relation_person_name_owner');
  ...
  final tenantRelationPerson =
  safeString(data, 'relation_person_name_tenant');
  final tenantPermAddress =
  safeString(data, 'permanent_address_tenant');
  ...
  final fullAddress = [bhk, floor, address]
      .where((e) => e != null && e.toString().trim().isNotEmpty)
      .join(', ');

  final monthlyRentRaw = data['monthly_rent']?.toString() ?? '';
  final customMaintenanceRaw =
  ...
  final String maintenanceAmount = maintenanceType == 'excluding'
  ...
  maintenanceType == 'excluding' ? 'excluding' : 'including';
...
// ✅ NEW PARAMETERS: Reference Number & eStamping Certificate
  ...
  pw.TextSpan getMeterClause(Map<String, dynamic>? data) {
    ...
    final meterValue =
    (customMeterUnit != null &&
        customMeterUnit.toString().isNotEmpty)
  ...
  try {
    if (data['shifting_date'] is Map &&
      ...
      shiftingDate =
          DateTime.parse(data['shifting_date']['date'].toString());
  ...
  final rentDueDay =
  shiftingDate != null ? getDayWithSuffix(shiftingDate.day) : '';
  ...
  pdf.addPage(
    ...
    eStamping_Certificate_No,
      ...
      build: (context) => [
        ...
        pw.Partition(
          child: pw.Column(
            ...
            children: [
              ...
              pw.Text(
                  '$ownerName, $ownerRelation, $ownerRelationPerson, Resident of $ownerAddress (hereinafter called the FIRST PARTY/LANDLORD).',
                  style: boldStyle),
              ...
              pw.SizedBox(height: 10),
              pw.Text(
                  '$tenantName, $tenantRelation, $tenantRelationPerson, Resident of $tenantPermAddress (hereinafter called the SECOND PARTY/TENANT).',
                  style: boldStyle),
            ],
          ),
        ),
        ...
        if (additionalTenants.isNotEmpty) ...[
          ...
          pw.Text(
            additionalTenants.asMap().entries.map((entry) {
              ...
              final relationPerson =
                  t['relation_person_name_tenant'] ?? '';
        ...
        pw.SizedBox(height: 6),
        pw.Text(
            'And the tenant also has agreed to take on rent the said property on the following terms and conditions of this Leave & License:-',
            style: baseStyle),
        pw.SizedBox(height: 10),
        pw.Text('NOW THIS LEAVE & LICENSE WITNESSETH AS UNDER:',
            style: pw.TextStyle(
              ...
              text: pw.TextSpan(
                ...
                const  pw.TextSpan(
                 ...
                 const pw.TextSpan(
                  ...
                  pw.TextSpan(
                    text:
                    '( ${numberToWords(maintenanceInt)} RUPEES ).',
                    style: boldStyle,
                  ),
                ],
              ),
        ...
        ...(() {
          ...
          (c['title'] ?? '').trim().isNotEmpty ||
              ...
              .toList();
          return valid
              .map((c) => customClausePdf(c['title'] ?? '', c['subtitle'] ?? ''))
              .toList();
        })(),
        ...
        clause('Taxes and Landlord\'s Liability:',
            'The Landlord is responsible for paying house and municipal taxes. However, the Landlord isn\'t liable for disputes between the Tenant and other residents, or for any loss, damage, or injury to the Tenant\'s personal belongings or to the Tenant and their guests.'),

        clause('Damage, Handover, and Last Month Rent Adjustment:',
            'The Tenant shall keep the premises in good, clean, and well-maintained condition. Any damage beyond normal wear and tear shall be repaired at the Tenant\'s cost. At the end of the tenancy, the Tenant must vacate and hand over the premises in the same condition as received. The Tenant shall not adjust or deduct the last month\'s rent from the security deposit under any circumstances. The security deposit will be refunded only after final inspection, verification of damages, and settlement of all outstanding dues including rent, electricity, cleaning charges, or repair expenses. Delay or failure to hand over possession, or refusal to pay for damages, will make the Tenant liable for penalties, recovery proceedings, and legal action under IPC Sections 441, 447, and 427 (BNS 329(5), 330, 303).'),

        clause('Vacating & Termination:',
            'The Tenant must vacate the premises in its original condition, with whitewashing done before leaving. Either party may terminate the agreement by giving one month\'s written notice. If the tenancy is extended beyond the initial term, the monthly rent shall increase by 10%, unless both parties agree otherwise in writing. (IPC 441, 447) (BNS 329(5), 330)'),

        clause('Misconduct, Noise, and Community Disturbance:',
            'The Tenant shall not engage in loud music, shouting, fighting, or any activity that disturbs neighbors, disrupts peace, or causes trouble in the society or locality. Any unlawful, improper, socially unacceptable behavior, repeated complaints from neighbors, or disturbance of public tranquility shall be treated as misconduct. In such cases, the Landlord shall have the right to terminate the tenancy by giving one month\'s written notice. After the notice period ends, the Tenant must vacate the premises without objection. Violations under this clause may attract action under IPC Sections 268, 290, 504, 506, and 323. (BNS 280, 356, 351, 115)'),

        clause('Tenant Information:',
            'The Tenant and their visitors must provide valid ID and required details to the Owner before moving in. In case of disputes or misconduct, the Owner may evict the Tenant. The Owner also reserves the right to terminate the agreement with one month\'s notice before the term ends. (IPC 188, 177) (BNS 223(3), 223(2))'),

        clause('Prohibited Activities:',
            'The Second Party shall not engage in illegal or unlawful activities in the premises. The Second Party shall not keep any unauthorized inflammable or explosive items in the premises. (IPC 285, 286, 188, 120B) (BNS 281(2), 282, 223(3), 61)'),

        clause('Loan & Credit Application:',
            'The Second Party will not apply for any loan or credit card using the said address. If any loan is pending against the Second Party, the First Party will not be liable or responsible for the same. (IPC 420, 468, 471)  (BNS 318, 326, 327(2))'),

        clause('Restriction on GST Registration:',
        ...
        clause('Repair and Cleanliness:',
            'The Tenant is responsible for minor, day-to-day repairs at their own expense. They must return the premises in the same condition as received and keep it clean and hygienic. (IPC 268) (BNS 280)'),

        clause('Liability for Death or Suicide:',
            'In the event of any death, suicide, or injury occurring within the premises, the First Party/Landlord shall not be held responsible or liable for any claims arising therefrom.'),
        ...
        clause('Fire Safety & Hazardous Materials Prohibition:',
            'The Tenant shall not store flammable, chemical, explosive, or hazardous materials. Violation may attract action under IPC Section 285 and IPC Section 286, and may result in immediate termination.(IPC 285, 286) (BNS 281(2), 282)'),

        clause('Restriction on Subletting & Unauthorized Occupancy:',
            'The Second Party/Tenant shall not sublet, share, transfer, or allow any third party to occupy the premises or any portion of it without the prior written consent of the First Party/Landlord. No person other than the Tenant is permitted to stay, reside, or use the premises in any manner without such written approval. Any form of unauthorized occupancy, shared accommodation, subletting, or permitting outsiders to stay whether temporary or permanent shall be treated as a violation of this Agreement and may result in immediate termination and legal action under IPC Sections 441 and 447. (BNS 329(5), 330)'),

        clause('Living Relationships:',
        ...
        clause('Disputes:',
            'All disputes or legal matters arising out of this Leave & License shall be settled exclusively in the court that has lawful jurisdiction over the area in which the rented property is situated. Only the court designated by the Government for this locality shall have the authority to hear and decide such cases.'),

        clause('Digital Communication & E-Signature Validity:',
            'Both parties agree that communication done through WhatsApp, Email, SMS, or any digital platform shall be considered valid. Digital signatures or scanned signatures shall be treated as legally binding under the Information Technology Act, 2000. (IPC 465, 471) (BNS 325(1), 327(2))'),
  ...
  // ─── PAGE 2 — POLICE VERIFICATION LETTER ─────────────────────────
  pdf.addPage(
    pw.MultiPage(
      footer: (context) {
        return pw.Container(
      ...
      build: (context) => [
            ...
            text: pw.TextSpan(
              children: [
                ...
                if (additionalTenants.isEmpty) ...[
                  ...
                  pw.TextSpan(
                      text: ', $tenantRelation ', style: baseStyle),
                  pw.TextSpan(
                      text: tenantRelationPerson,
                      style: boldStyle),
                ] else ...[
                ...
                pw.TextSpan(
                    text: '. The owner of the property is ',
                    style: baseStyle),
                pw.TextSpan(text: ownerName, style: boldStyle),
                pw.TextSpan(
                    text: ', $ownerRelation ', style: baseStyle),
                pw.TextSpan(
                    text: ownerRelationPerson, style: boldStyle),
                pw.TextSpan(
                    text: ' residing at ', style: baseStyle),
                pw.TextSpan(
                    text: ownerAddress, style: boldStyle),
                pw.TextSpan(text: '.\n', style: baseStyle),
              ],
            ),
          ),
        ...
        pw.SizedBox(height: 20),
        pw.Text('Thank you for your attention to this matter.',
            style: baseStyle),
        pw.SizedBox(height: 12),
  ...
  final ownerFrontImg = await _loadAgreementImage(
      data['owner_aadhar_front']?.toString() ?? '');
  final ownerBackImg = await _loadAgreementImage(
      data['owner_aadhar_back']?.toString() ?? '');
  final tenantFrontImg = await _loadAgreementImage(
      data['tenant_aadhar_front']?.toString() ?? '');
  final tenantBackImg = await _loadAgreementImage(
      data['tenant_aadhar_back']?.toString() ?? '');

  print(
      '🖼️ ownerFrontImg  = ${ownerFrontImg != null ? "LOADED" : "NULL"}');
  print(
      '🖼️ ownerBackImg   = ${ownerBackImg != null ? "LOADED" : "NULL"}');
  print(
      '🖼️ tenantFrontImg = ${tenantFrontImg != null ? "LOADED" : "NULL"}');
  print(
      '🖼️ tenantBackImg  = ${tenantBackImg != null ? "LOADED" : "NULL"}');

  pw.Widget _imgWidget(
      pw.ImageProvider? imgProvider, String label) {
    if (imgProvider != null) {
      return pw.Expanded(
      ...
      build: (_) => pw.Column(
        ...
        children: [
          ...
          pw.Row(
            crossAxisAlignment: pw.CrossAxisAlignment.start,
            children: [
              _imgWidget(ownerFrontImg, 'Owner Front'),
              pw.SizedBox(width: 10),
              _imgWidget(ownerBackImg, 'Owner Back'),
            ],
          ),
          pw.SizedBox(height: 20),
          ...
          pw.Row(
            crossAxisAlignment: pw.CrossAxisAlignment.start,
            children: [
              _imgWidget(tenantFrontImg, 'Tenant Front'),
              pw.SizedBox(width: 10),
              _imgWidget(tenantBackImg, 'Tenant Back'),
            ],
          ),
        ],
      ),
  ...
  // ─── ADDITIONAL TENANTS — each on separate page ───────────────────
  if (additionalTenants.isNotEmpty) {
    for (final t in additionalTenants) {
      final String tFrontPath =
      (t['tenant_aadhar_front']?.toString().isNotEmpty == true
          ? t['tenant_aadhar_front']
          : t['tenant_aadhaar_front'] ?? '')
          .toString();
      final String tBackPath =
      (t['tenant_aadhar_back']?.toString().isNotEmpty == true
          ? t['tenant_aadhar_back']
          : t['tenant_aadhaar_back'] ?? '')
          .toString();

      final tFrontImg = await _loadAgreementImage(tFrontPath);
          ...
          build: (_) => pw.Column(
            ...
            children: [
              ...
              pw.Row(
                crossAxisAlignment: pw.CrossAxisAlignment.start,
                children: [
                  _imgWidget(tFrontImg, '$tName Front'),
                  pw.SizedBox(width: 10),
                  _imgWidget(tBackImg, '$tName Back'),
                ],
              ),
            ],
          ),
  ...
  final file = File(
      '${dir.path}/agreement_${data['id'] ?? 'unknown'}.pdf');
