...
class InsuranceProject {
  ...
  InsuranceProject.fromJson(Map<String, dynamic> j)
      : id = j['id'],
        name = j['name_'] ?? '',
        number = j['number'] ?? '',
...
Future<InsuranceProject> fetchProject(int id) async {
  final uri = Uri.parse(
  ...
  return InsuranceProject.fromJson(json['data'][0]);
}
...
// ─── MAIN SCREEN ─────────────────────────────────────────────────────────────
class PdfQuotationScreen extends StatefulWidget {
  final int projectId;
  const PdfQuotationScreen({super.key, required this.projectId});

  @override
  State<PdfQuotationScreen> createState() => _PdfQuotationScreenState();
}

class _PdfQuotationScreenState extends State<PdfQuotationScreen> {
  late Future<InsuranceProject> _future;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: FutureBuilder<InsuranceProject>(
        future: _future,
        builder: (ctx, snap) {
...
class _ProjectBody extends StatelessWidget {
  final InsuranceProject project;
