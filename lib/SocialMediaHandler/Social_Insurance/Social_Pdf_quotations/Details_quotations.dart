...
import 'Social_Insurence_form.dart';

// ── Model (same as before) ───────────────────────────────────
class SocialInsurancePlan {
  final String subid;
  ...
  factory SocialInsurancePlan.fromJson(Map<String, dynamic> json) {
    return SocialInsurancePlan(
      subid: json['subid'] ?? '',
...
// ── Screen ───────────────────────────────────────────────────
class SocialInsurancePlansScreen extends StatefulWidget {
  final String Id; // ← subid pass karo parent se

  const SocialInsurancePlansScreen({
    super.key,
    required this.Id,
  ...
  @override
  State<SocialInsurancePlansScreen> createState() => _SocialInsurancePlansScreenState();
}

class _SocialInsurancePlansScreenState extends State<SocialInsurancePlansScreen> {
  List<SocialInsurancePlan> plans = [];
  ...
  Future<void> _fetchPlans() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          plans = jsonData.map((e) => SocialInsurancePlan.fromJson(e)).toList();
          _isLoading = false;
        });
      } else {
        setState(() {
  ...
  Widget _buildPlanCard(SocialInsurancePlan plan, int index) {
