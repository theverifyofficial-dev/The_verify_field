...
class Catid_tenant {
  ...
  Catid_tenant(
      {required this.V_name, required this.V_number, required this.demand_id});

  factory Catid_tenant.FromJson(Map<String, dynamic>json){
    return Catid_tenant(
        V_name: json['tenant_name'],
        V_number: json['tenant_number'],
        demand_id: json['demand_id']);
  }
}

class Details_Fullfilled_Demand extends StatefulWidget {
  String id;
  Details_Fullfilled_Demand({super.key, required this.id});

  @override
  State<Details_Fullfilled_Demand> createState() => _Details_Fullfilled_DemandState();
}
        ...
        child: SingleChildScrollView(
          child: Column(
            children: [
              ...
              Text("Tenant for Other Property",style: TextStyle(color: Colors.white),),

              Container(
                child: FutureBuilder<List<Catid_tenant>>(
                    future: fetchData_Tenant(""+1.toString()),
                    builder: (context,abc){
                      if(abc.connectionState == ConnectionState.waiting){
                        return Center(child: CircularProgressIndicator());
                      }
                            ...
                            child: ElevatedButton(
                              ...
                              onPressed: () {
                                Navigator.push(
                                    context,
                                    MaterialPageRoute(
                                        builder: (context) => Add_tenant_under_demandfullfilled(id: '${abc.data![len].id.toString()}',)));
                                // Button 1 action
                                print('Button 1 pressed');
                              },
                              child: Text('Add Tenant',style: TextStyle(fontSize: 15),),
                            ),
                          );
                        });
                  }
